  ...
  async execute(interaction: any) {
    ...
    if (!signups.has(channelId)) {
      return interaction.reply({
        content: `No signups found for this channel.`,
        ephemeral: true,
      });
    }
    ...
    const mainListIndex = mainList.findIndex(
      (team) => team.teamName === teamName && isUserInTeam(team),
    );
    if (mainListIndex !== -1) {
      ...
      if (waitlistIndex !== -1) {
        ...
        return interaction.reply({
          content: `Team ${teamName} not found in the signups or you are not a member of this team.`,
          ephemeral: true,
        });
      }
    }
    ...
    await interaction.reply(
      `Team ${teamName} has been dropped from the signups.`,
    );
  },
};
